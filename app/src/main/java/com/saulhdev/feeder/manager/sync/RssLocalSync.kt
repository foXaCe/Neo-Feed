                ...
                supervisorScope {
                    ...
                    val staleTime: Long =
                        if (forceNetwork) {
                            ...
                            Clock.System
                                .now()
                                .minus(minFeedAgeMinutes.toLong(), DateTimeUnit.MINUTE)
                                .toEpochMilliseconds()
                        }
...
private suspend fun syncFeed(
    ...
    feedsRepo.updateSource(
        syncedFeed.copy(
            ...
            feedImage =
                feed.icon?.let { sloppyLinkToStrictURLNoThrows(it) }
    ...
    val ids =
        articleRepo.getItemsToBeCleanedFromFeed(
            feedId = syncedFeed.id,
...
internal suspend fun feedsToSync(
    ...
    when {
        feedId > 0 -> {
            val feed =
                if (staleTime > 0) {
                    ...
                    repository.loadFeedById(feedId)
                }
            if (feed != null) {
                listOf(feed)
            } else {
        ...
        feedId == ID_ALL -> {
            repository.loadFeedIds().mapNotNull {
                if (staleTime > 0) {
                    repository.loadFeedIfStale(
                        feedId = it,
                        staleTime = staleTime,
